...
### 真实 Worker 可选接入（2026-07-20）

3 条项目二只读 A/B 仍是历史兼容性证据。此后已完成一次独立、有限的真实 Arq Worker 运行时接入验证：

- `USE_FINETUNED_MODEL` 默认仍为 `false`；显式启用时只替换 Researcher。
- Planner、Analyst、Writer 继续使用 DeepSeek V4 Pro，MiMo ASR 不受影响。
- App 与 Worker 共用可选路由初始化；`qwen3-tool-calling` 只是 OpenAI 兼容服务模型 ID，实际加载 Qwen3-4B + v4.1 Direct Adapter。
- 有效 canary 中 Planner 调用 DeepSeek 1 次，Researcher 到达本地服务 2 次且 HTTP `200/200`，两次 JSON 均成功解析，`invalid_params=0`。
- 最终 `partial / EVIDENCE_INSUFFICIENT` 来自外部工具返回 `empty`，不是 Worker、模型路由或结构化解析失败。
- 关闭开关后 Researcher 覆盖被清除，本地请求数不再增长。

这只是一轮有限 canary：没有证明生产稳定性、并发吞吐、成本、延迟或端到端产品质量更优，也没有替换项目二默认 DeepSeek。该接入没有重新训练、继续微调、合并 Adapter 或重新量化。
...
基座通过 `BASE_MODEL_PATH` 指向已有的本地 Qwen3-4B 缓存，例如：
...
- 项目二接入：已完成默认关闭的真实 Worker 可选路由，仅允许切换 Researcher；默认模型继续使用 DeepSeek V4 Pro。
...
```powershell
$env:BASE_MODEL_PATH='<local-qwen3-4b-path>'
...
- 以默认关闭的可选路由接入真实 Arq Worker，并完成有限路由、解析和回退 canary。
...
- 已完成生产部署，或有限 canary 证明了并发、稳定性、成本、延迟和产品质量优势。
