...
`researcher_prompt.py` 是评测与本地服务共用的完整 Prompt 入口。当前评分会归一化平台别名、平台列表顺序、可省略默认参数和等价检索词，并单列 JSON 有效率、延迟与“臆造视频 ID/URL”安全指标。
...
| SFT+DPO v3 + base | hard44 | 95.5% | 83.3% | 86.4% | 79.5% | 97.7% | 1791 ms |
| SFT+DPO v3 + base | holdout20 | 100.0% | 100.0% | 100.0% | 100.0% | 100.0% | 1878 ms |
| SFT+DPO v3 + rules | hard44 | 100.0% | 75.0% | 86.4% | 77.3% | 100.0% | 1896 ms |
| SFT+DPO v3 + rules | holdout20 | 80.0% | 60.0% | 70.0% | 70.0% | 100.0% | 1894 ms |

结果文件：`results/fair_experiment_20260711_152552_rescored.json`。

这轮实验得到两个负结论：第一，Base 与 SFT+DPO v3 在对应 Prompt/数据集上的汇总指标完全相同，仍不能证明微调带来自然表达泛化提升；第二，显式路由规则只在 hard44 上提高工具名准确率，却让独立 holdout20 从 100% 降到 80%，因此不能把规则版直接设为默认。hard44 中还发现模型会在未提供链接时生成示例视频 URL，说明“工具选对”不等于“参数安全可执行”。

### 实验结论

当前小样本 LoRA 证明了分布内格式学习和已覆盖场景有效：内置 50 条 BFCL 风格用例从 88% / 54% 提升到 94% / 80%。但历史 hard/holdout 与本轮同 Prompt 公平实验都没有显示 SFT+DPO 超过基座，不能证明自然表达边界泛化提升。
